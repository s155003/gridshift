...
GridShift's core insight is that **you don't need a carbon API — you need weather.**
...
encoded in the features rather than left for the trees to rediscover — a real
...
at all** — a deterministic parser takes over — so the demo can't break on a network
call.
...
- Transfer to non-GB regions is **not validated** — no ground truth exists to check
  it. The shape should transfer (it's physics); the level is a calibration. Labelled
  `transferred` everywhere it appears in the UI.
- Uses **average**, not marginal, carbon intensity — average is what's openly
  published, but marginal is arguably the correct signal for a shifting decision.
- The EV archetype remains weak (0.03). Where a real carbon API exists, use it —
  GridShift does exactly that for GB.
